...
function authenticate_and_authorize(
    username,
    password,
    action,
    resource_id,
    ...
    fetch(AUTHENTICATION_URL, {
        ...
        .then((json) => {
            if (json["status"]) {
                ...
                fetch(OPA_TENANT_URL, {
                    ...
                    .then((json) => {
                        ...
                        if (json["result"]["allow"]) {
                            console.log(
                                "Tenant %s is authorized, action: %s, topic: %s",
                                username,
                                action,
                                iot_resource_id
                            );
                            ...
                            console.log(
                                "Tenant %s is not authorized, action: %s, topic: %s",
                                username,
                                action,
                                iot_resource_id
                            );
                ...
                console.log(
                    "Tenant %s is authenticated, action: %s, topic: %s",
                    username,
                    action,
                    resource_id
                );
